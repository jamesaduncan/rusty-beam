...
fn main() {
    ...
    match extractor.extract(&content) {
        Ok(items) => {
            ...
            for item in &items {
                if item.item_type() == Some("http://rustybeam.net/AuthorizationRule") {
                    ...
                    println!("Rule {}: user='{}', role='{}', path='{}', action='{}', methods={:?}", 
                             rules_count, username, role, path, action, methods);
            ...
            // Check if there's any rule that should match a request to "/"
            println!("\n=== Rules that should match request to '/' ===");
            for item in &items {
                if item.item_type() == Some("http://rustybeam.net/AuthorizationRule") {
                    let username = item.get_property("username").unwrap_or_default();
                    let role = item.get_property("role").unwrap_or_default();
                    ...
                    if path_matches("/", &path) && methods.contains(&"GET".to_string()) {
                        println!("Match: user='{}', role='{}', path='{}', action='{}', methods={:?}",
