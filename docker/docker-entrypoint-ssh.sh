...
if [ ! -z "$DOCS_GIT_REPO" ]; then
    ...
    # Check if it's an SSH URL and SSH key is provided
    if [[ "$DOCS_GIT_REPO" == git@* ]] || [[ "$DOCS_GIT_REPO" == *:*/* ]]; then
        if [ ! -z "$GIT_SSH_KEY" ]; then
            ...
            # Add GitHub/GitLab/Bitbucket to known hosts
            ...
            echo "ERROR: SSH URL detected but GIT_SSH_KEY environment variable not set"
            echo "Either provide GIT_SSH_KEY or use HTTPS URL"
            exit 1
        fi
    fi
